...
<link href="css/style-page.css" rel="stylesheet" />        
	...
	<div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		...
		<div class="modal-body">
            ...
            <!--<div class="content">-->
              <form class="form-horizontal cmxform" id="linkbFR" method="get" action="proses/pmaster.php" autocomplete="off">
              	<input type="hidden" id="idform" />
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3">linkb </label>
...
<script language="javascript" type="text/javascript" src="js/plugins/avgrund.js"></script> 
	<div class="row-fluid">
        <div class="box color_25 ">
          <div class="title">
            <h4> <span>Data Master - link B </span>
           </h4>&nbsp;
				<select id="kategoriTB">
					<option value="nama" selected="selected">link B</option>
                ...
                &nbsp;
                 <?php if($_SESSION['leveluserMUX']=='admin'){?>
					...
					<?php }?>
						<a id="cetakBC"  data-original-title=" Cetak Semua " data-placement="top" rel="tooltip"class="btn " href="report/cmaster.php?tipe=full&tabelx=tb_linkb&judulx=linkb" target="_blank"><i class="icon-print"></i></a>
...
<script type="text/javascript">
	...
	{	
		...
		function loadData(){
			...
			$.ajax({
				url	: proses+"?aksi=tampil&menu=mlinkb",
				type: "GET",
				data: dataString,
		...
		//simpan(tambah & edit)
		$('#linkbFR').submit(function() {
			var idformx = $("#idform").val();
			var urlx 	= $(this).attr('action');

			if(idformx==''){ //tmbah data
				urlx2 = "?aksi=tambah&menu=mlinkb";
			}
			else{ //edit data
				urlx2 = "?aksi=ubah&menu=mlinkb&idx="+idformx;
			}
			$('#hasily').html("loading ....");
			$.ajax({
				type: 'POST',
				...
				success: function(data) {
					...
					if(statusy=='sukses'){
						...
						if(idformx==''){
							...
							//alert(idy);
							loadData();
							$("#linkb_"+idy).css("background","green");
						}
						//edit
						else{
							$('#myModal').modal('hide');
							var idy = idformx;
							$("#linkb_"+idy).css("background","blue");
							$("#loadtabel").html('update <br> linkb : "'+namay+'" <br> keterangan : '+keterangany).fadeIn(6000);
							loadData();
						}
						kosongkan();
						$('#hasily').hide();
		...
		$('a.edit').live("click",function() {	
			...
			$.ajax({
				url:proses+"?aksi=ambiledit&menu=mlinkb",
				data:"idx="+idy,
				dataType:'json',
				type:"GET",
				success:function(data){
		...
		$('i.view').live("click",function() {	
			...
			$.ajax({
				...
				dataType:'json',
				url:proses,
				data:"aksi=cetak&menu=mlinkb&idx="+idy,//+"&namax="+namay+"&keteranganx="+keterangany,
				success:function(data){
					var linkby 		= data.nama;
					var keterangany	= data.keterangan;
					var bodyy		='';
						bodyy		+='<div align=center><b>Detail linkb (perusahaan)</b></div>';
						bodyy		+='<table align=center >';	
						bodyy		+='<tr><td>linkb</td><td> :'+ linkby+'</td></tr>';
		...
		$('a.hapus').live("click",function() {	
			...
			if(confirm("lanjutkan menghapus '"+namay+"' ?"))
				...
				$.ajax({
						type: "GET",
						dataType:"json",
						url: proses+"?aksi=hapus&menu=mlinkb",
						data:"idx=" + idy,
						cache: false,
						success: function(data){
							$('#loadtabel').html('<img src="img/w8.gif">').fadeOut();
							...
							$("#linkb_"+idy).css("background","yellow").fadeOut(2000);
							loadData();
							$("#loadtabel").html('linkb : "'+namay+'" terhapus').fadeIn(6000);
						}
				});
		});
		//end of action hapus 
		...
		$('#hapusBC').click(function() {	
			if(confirm("lanjutkan menghapus semua linkb  ?"))
				$.ajax({
						type: "GET",
						dataType:'json',
						url: proses,
						data:"aksi=hapussemua&menu=mlinkb",
						success: function(data){
		...
		$('#cetakBCx').click(function(){
			$.ajax({
				type:'post',
				url:'lib/tespddf.php',
				//data :'tabelx=tb_linkb&idx=id_linkbkerja',
				data :'tabelx=tb_linkb',
				success:function(data){
					//opener = $('<div>').append(print);
		...
		$('#cetakBCz').click(function(e) {
			...
			jQuery.ajax({
				...
				url: "lib/tespddf.php",
				data: "tabelx=tb_linkb",
				//data: "tabelx=tb_linkb&ancur="+md5,
