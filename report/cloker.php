<?php 
	...
	if($_GET['tabelx'])
	{
		...
		$idy 	= $_GET['id_loker'];
		$tabel	= $_GET['tabelx'];	
		$query 	= "SELECT * FROM $tabel where id_loker ='$idy'";
		//var_dump($idy);exit();
		...
		$pdf->Cell($w1, 8, 'ID Lokasi Kerja', 0, '0', 'L', true);
		$pdf->Cell($w2, 8, ':  '.$data['id_loker'], 0, '0', 'L', $f1);
		$pdf->Ln();

		$pdf->Cell($w1, 8, 'Lokasi Kerja', 0, '0', 'L', $f2);
		$pdf->Cell($w2, 8, ':  '.$data['loker'], 0, '0', 'L', $f2);
		$pdf->Ln();
