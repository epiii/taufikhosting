<?php 
	...
	if($_GET['tabelx'])
	{
		...
		$idy 	= $_GET['id_user'];
		$tabel	= $_GET['tabelx'];	
		$query 	= "SELECT * FROM $tabel where id_user = '$idy' ";
		//var_dump($idy);exit();
		...
		$pdf->Cell($w1, 8, 'ID User', 0, '0', 'L', true);
		$pdf->Cell($w2, 8, ':  '.$data['id_user'], 0, '0', 'L', $f1);
		$pdf->Ln();

		$pdf->Cell($w1, 8, 'User/Pelanggan', 0, '0', 'L', $f2);
		$pdf->Cell($w2, 8, ':  '.$data['user'], 0, '0', 'L', $f2);
		$pdf->Ln();
